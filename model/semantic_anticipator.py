...
class BaseModel(nn.Module):
    ...
    def __init__(self, cfg=None):
        ...
        if cfg.OUTPUT_NORMALIZATION.channel_0 == "sigmoid":
            self.normalize_channel_0 = torch.sigmoid
        elif cfg.OUTPUT_NORMALIZATION.channel_0 == "softmax":
            self.normalize_channel_0 = softmax_2d

        if cfg.OUTPUT_NORMALIZATION.channel_1 == "sigmoid":
            self.normalize_channel_1 = torch.sigmoid
        elif cfg.OUTPUT_NORMALIZATION.channel_1 == "softmax":
...
class SemAntRGBD(BaseModel):
    ...
    def _create_gp_models(self):
        nmodes = 2
        gp_cfg = self.config.SEMANTIC_ANTICIPATOR
    ...
    def _do_gp_anticipation(self, x):
        ...
        Inputs:
            x is a dictionary containing the following keys:
                ...
                'ego_dyn' - (bs, 2, H, W) probabilities
        """
...
class SemAntGroundTruth(BaseModel):
    ...
    def _do_gp_anticipation(self, x):
        x_dec = x["ego_dyn_anticipated"]  # (bs, 1, H, W)
        outputs = {"sem_estimate": x_dec}
...
class SemAnticipator(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.config = cfg
        model_type = cfg.SEMANTIC_ANTICIPATOR.type
...
class SemAnticipationWrapper(nn.Module):
    ...
    def forward(self, x):
        x["rgb"] = padded_resize(x["rgb"], self.input_hw[1])
        if "ego_dyn" in x:
            x["ego_dyn"] = F.interpolate(x["ego_dyn"], size=[self.input_hw[1],self.input_hw[1]])
        x_full = self.main(x)
